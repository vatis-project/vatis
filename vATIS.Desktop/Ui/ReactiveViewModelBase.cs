...
public abstract class ReactiveViewModelBase : ReactiveObject, INotifyDataErrorInfo
{
    ...
    {
        if (!this.errors.TryGetValue(propertyName, out List<string>? value))
        {
            value = [];
    ...
    {
        var propertyNames = new List<string>(this.errors.Keys);
        this.errors.Clear();
    ...
    {
        return this.errors.GetValueOrDefault(propertyName);
    }
